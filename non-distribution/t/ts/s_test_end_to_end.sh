...
cat "$STUDENT_DATA"/d/u.txt > d/urls.txt

./engine.sh

EXIT=0

if $DIFF <(sort d/visited.txt) <(sort "$STUDENT_DATA"/v.txt) >&2;
then
...
if DIFF_PERCENT=$DIFF_PERCENT t/gi-diff.js <(sort d/global-index.txt) <(sort "$STUDENT_DATA"/i.txt) >&2;
then
