...
const printMerged = (err, data) => {
  ...
  for (const line of globalIndexLines) {
    ...
    for (let i = 0; i < strArray.length; i += 2) {
      const url = strArray[i];
      const freq = parseInt(strArray[i + 1], 10);
    ...
    global[word] = grouped; // Map<url, freq>
  }
  ...
  const sortedKeys = Object.keys(global).sort();
